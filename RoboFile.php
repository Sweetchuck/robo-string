...
class RoboFile extends Tasks implements LoggerAwareInterface, ConfigAwareInterface
{
    ...
    /**
     * @phpstan-var dev-composer-info
     */
    protected array $composerInfo = [];

    /**
     * @phpstan-var dev-condeception-info
     */
     ...
     * @initLintReporters
     */
    public function lintPhpstan(): CollectionBuilder
    {
        return $this
            ->collectionBuilder()
            ->addTask($this->getTaskPhpstanAnalyze());
    ...
    /**
     * @phpstan-param dev-php-executable-array $php
     */
    protected function getTaskCodeceptRunSuite(string $suite, array $php): CollectionBuilder
    {
        ...
        if ($withCoverageHtml) {
            ...
            $cb->addTask(
                $this
                    ->taskFilesystemStack()
                    ->mkdir("$outputDir/human/coverage/$suite")
            );
        }
        ...
        if ($withCoverageHtml || $withCoverageXml) {
            ...
            $cb->addTask(
                $this
                    ->taskFilesystemStack()
                    ->mkdir("$outputDir/machine/coverage/$suite")
            );
        }
